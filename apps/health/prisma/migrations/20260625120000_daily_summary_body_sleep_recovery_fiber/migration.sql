-- Surface already-collected data the view never exposed: body composition
-- (body_fat_pct, muscle_mass_kg from Withings), sleep depth (deep_min, rem_min),
-- recovery (hrv_ms, resting_hr_bpm from sleep sessions), and fiber. Additive,
-- CREATE OR REPLACE VIEW only (non-destructive). Every new column is appended LAST:
-- CREATE OR REPLACE VIEW cannot reorder/rename/drop existing view columns, only add
-- at the end. No calorie/expenditure semantics change — this is purely surfacing.
...
-- weight_daily/weight_avg now also carry body_fat_pct and muscle_mass_kg (latest
...
-- sleep_daily now also rolls up sleep depth (summed across the day's sessions) and
